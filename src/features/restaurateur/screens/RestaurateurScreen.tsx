...
const RestaurateurScreen: React.FC<RestaurateurScreenProps> = ({ onLogout, dishes, onUpdateDishes }) => {
  ...
  const convertTikTokUrl = async (url: string) => {
    if (!url || !url.includes('tiktok.com')) return;
    if (url.includes('tiktokcdn.com') || url.includes('cloudinary.com')) return;
    if (isConvertingVideo) return; // Prevent multiple simultaneous calls

    setIsConvertingVideo(true);
    try {
      const response = await fetch('http://localhost:3001/api/get-direct-link', {
        method: 'POST',
      ...
      if (data.success && data.videoUrl) {
        setNewDish(prev => ({ ...prev, videoUrl: data.videoUrl }));
      } else {
        console.error("Conversion failed:", data.error);
        alert("Impossible de convertir ce lien TikTok. Vérifiez qu'il est valide.");
      }
    } catch (error) {
  ...
  const handleVideoUrlChange = (val: string) => {
    setNewDish({ ...newDish, videoUrl: val });

    // Trigger conversion if it's a complete TikTok link
    // We check for length and common patterns to catch paste events immediately
    const isFullUrl = val.includes('tiktok.com/') && (val.includes('/video/') || val.includes('vm.tiktok.com') || val.length > 25);

    if (isFullUrl) {
      convertTikTokUrl(val);
    }
  };
          ...
          <div className="bg-white w-full max-w-sm rounded-[2rem] p-6 shadow-2xl overflow-y-auto max-h-[90vh] animate-in fade-in zoom-in duration-200">
            ...
            <div className="space-y-5">
              ...
              <div className="bg-orange-50 p-6 rounded-3xl border border-orange-100/50">
                <label className="block text-xs font-black text-orange-600 mb-2.5 flex items-center gap-2 uppercase tracking-widest">
                  <Video size={14} />
                  {isConvertingVideo ? 'CONVERSION DU LIEN...' : 'LIEN VIDÉO (TIKTOK/MP4)'}
                  {isConvertingVideo && <Loader2 size={14} className="animate-spin" />}
                </label>
                <div className="relative group">
                  <input
                    ...
                    className={`w-full bg-white border rounded-2xl p-4 focus:ring-4 focus:ring-orange-100 outline-none text-sm text-gray-900 placeholder:text-gray-400 font-semibold transition-all shadow-sm ${isConvertingVideo ? 'opacity-70 cursor-wait border-orange-300 ring-2 ring-orange-200' : 'border-orange-100 hover:border-orange-200'}`}
...
const NavButton = ({ icon, label, active, onClick }: { icon: React.ReactNode, label: string, active: boolean, onClick: () => void }) => (
  <button
    onClick={onClick}
    className={`flex flex-col items-center gap-1 p-3 rounded-xl transition-all duration-300 w-16 active:scale-95 ${active ? 'text-orange-500' : 'text-gray-400 hover:text-gray-600'
      }`}
  >
